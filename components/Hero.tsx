...
export function Hero({ data }: { data: Profile }) {
  return (
    <section className="flex flex-col items-center gap-6 pb-10 border-b border-foreground/10">
      <div className="relative size-24 sm:size-28 rounded-full overflow-hidden bg-foreground/10 shrink-0 ring-2 ring-foreground/10 ring-offset-2 ring-offset-background">
        <Image
          src={data.photo}
          alt={data.name}
          fill
      ...
      <nav className="flex items-center gap-2" aria-label="شبکه‌های اجتماعی">
        <a
